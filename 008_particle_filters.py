...
class robot:
    ...
    #           self.steering_noise
    #           self.distance_noise
    def move(self, motion, tolerances = 0.001): # Do not change the name of this function
        steering = motion[0]
        distance = motion[1]
        ...
        _steering = random.gauss(steering, self.steering_noise)
        _distance = random.gauss(distance, self.distance_noise)
        beta = (_distance / result.length) * tan(_steering)
        if abs(beta) < tolerances:
            result.x = self.x + _distance * cos(self.orientation)
            result.y = self.y + _distance * sin(self.orientation)
            result.orientation = (self.orientation + beta) % (2.0 * pi)
        else:
            radian = _distance / beta
            c_x = self.x - sin(self.orientation) * radian
            c_y = self.y + cos(self.orientation) * radian
            result.orientation = (self.orientation + beta) % (2.0 * pi)
            result.x = c_x + sin(result.orientation + beta) * radian
            result.y = c_y - cos(result.orientation + beta) * radian
        return result
    # --------
    # sense: 
    #    
    ...
    # according to
    #           self.bearing_noise
    def sense(self, add_noice = 1): #do not change the name of this function
        Z = []
        for lm in landmarks:
